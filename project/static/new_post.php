            ...
            <?php else : ?>
                ...
                <div class="text-center py-16">
                    <div class="bg-white dark:bg-gray-800 rounded-2xl p-12 shadow-lg border border-gray-200 dark:border-gray-700 max-w-md mx-auto">
                        ...
                        <div class="space-y-3">
                            <a href="?days=30" data-new-post-ajax="1" class="block w-full btn-primary">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function bindFormLoading(scope) {
            const container = scope || document;
        ...
        async function loadNewPostSection(url, pushState) {
            const root = document.getElementById('new-post-ajax-root');
            if (!root) {
            ...
            try {
                ...
                const nextRoot = parsed.getElementById('new-post-ajax-root');
                if (!nextRoot) {
                    window.location.href = url;
                    return;
                }
        ...
        document.addEventListener('click', function(event) {
            const anchor = event.target.closest('a[data-new-post-ajax="1"]');
            if (!anchor) {
                return;
            }
            ...
            loadNewPostSection(anchor.href, true);
        });

        window.addEventListener('popstate', function() {
            loadNewPostSection(window.location.href, false);
        });

        initCards(document);
        bindFormLoading(document);
    });
